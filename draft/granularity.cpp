...
// per-worker storage (i.e., thread local)
template <class Item>
class perworker {
private:
  Item x; // dummy implementation
public:
  Item& mine() {
    return x;
  }
...
perworker<int> unique_estimator_id;
...
// todo: these routines should look like the routines in `ticks.hpp`
...
public:
  ...
  control_by_cmdline(std::string name = "")
  : estimator(name), policy(By_prediction) { }
...
execmode_type cur_execmode;

// cutoff expressed in microseconds
cost_type kappa = 256.0;

template <class Body_fct>
void run_with_given_execmode(execmode_type c, const Body_fct& f) {
  execmode_type p = cur_execmode;
  cur_execmode = execmode_combine(p, c);
  f();
  cur_execmode = p;
}
...
void cstmt(control_by_force_parallel&, const Par_body_fct& par_body_fct) {
  run_with_given_execmode(Force_parallel, par_body_fct);
}
...
void cstmt(control_by_force_sequential&, const Seq_body_fct& seq_body_fct) {
  run_with_given_execmode(Force_sequential, seq_body_fct);
}
...
void cstmt(control_by_cutoff_without_reporting&,
  ...
  if (c == Sequential)
    ...
    run_with_given_execmode(Parallel, par_body_fct);
}
...
void cstmt(control_by_cutoff_with_reporting& contr,
  ...
  if (c == Sequential) {
    ...
    run_and_report_with_given_creg_config(m, seq_body_fct, estimator);
  } else {
    run_with_given_execmode(Parallel, par_body_fct);
  }
}
...
void cstmt(control_by_prediction& contr,
  ...
  if (c == Sequential)
    run_and_report_with_given_execmode(m, seq_body_fct, estimator);
  else
    run_with_given_execmode(Parallel, par_body_fct);
}
...
template <class Body_fct1, class Body_fct2>
void fork2(const Body_fct1& f1, const Body_fct2& f2) {
  if (     cur_execmode == Sequential
       ||  cur_execmode == Force_sequential ) {
    f1();  // sequentialize
    f2();
  } else {
    execmode_type c = cur_execmode;
    native::fork2([c,&f1] { run_with_given_execmode(c, f1); },
                  [c,&f2] { run_with_given_execmode(c, f2); });
  }
}
...
public:
  ...
  loop_by_eager_binary_splitting(std::string name = "anonloop")
  : gcpolicy(name) { }
...
public:
  ...
  loop_by_lazy_binary_splitting(std::string name = "anonloop")
  : gcpolicy(name) { }
...
public:
  ...
  loop_by_blocking(std::string name = "anonloop")
  : gcpolicy(name) { }
...
public:
  ...
  loop_by_multiple_of_nb_proc(std::string name = "anonloop")
  : gcpolicy(name) { }
...
public:
  ...
  loop_by_cmdline(std::string name = "")
  : gcpolicy(name) { }
  ...
  void set(std::string loop_algo_name) {
    if (loop_algo_name == "by_eager_binary_splitting")
      ...
      fatal([&] { std::cout << "bogus loop algorithm: " << loop_algo_name << std::endl; });
  }
...
template <class T> int foo<T>::x = 0;
